 ...
 * ("Ghana, Kigali" / "All · read only" / "Unassigned"). */
export async function getCohortScopesByUser(
  db: DB
): Promise<Map<string, { cohortName: string; capacity: string }[]>> {
  const { data, error } = await db
    .from("cohort_member")
    .select("user_id, capacity, cohort:cohort_id(name)");
  if (error) throw error;

  const map = new Map<string, { cohortName: string; capacity: string }[]>();
  for (const row of data ?? []) {
    const cohort = Array.isArray(row.cohort) ? row.cohort[0] : row.cohort;
    if (!cohort) continue;
    const list = map.get(row.user_id) ?? [];
    list.push({ cohortName: cohort.name, capacity: row.capacity });
